(() => {
  ...
  function restoreNeedsFromRoute(){
    ...
    needsSelected.clear();params.get('ids').split(',').filter(Boolean).forEach(id=>{if(window.NEEDS.some(n=>n.id===id))needsSelected.add(id);});
  }
  ...
  function renderNeeds(){
    get('#need-grid').innerHTML=window.NEEDS.map(need=>`<button class="need-option${needsSelected.has(need.id)?' is-selected':''}" type="button" data-need="${need.id}" aria-pressed="${needsSelected.has(need.id)}"><span class="need-check" aria-hidden="true">${needsSelected.has(need.id)?'✓':'+'}</span><span>${esc(need.label)}</span></button>`).join('');
  ...
  function renderResults(){
    const needs=window.NEEDS.filter(n=>needsSelected.has(n.id));
    if(!needs.length){navigate('needs',{replace:true});return;}
    const items=window.CATALOG.filter(item=>item.needs.some(id=>needsSelected.has(id)));
  ...
  function renderCatalog(){
    const categories=['Todas',...new Set(window.CATALOG.map(item=>item.category))];
    ...
    const items=activeCategory==='Todas'?window.CATALOG:window.CATALOG.filter(item=>item.category===activeCategory);
    get('#catalog-count').textContent=`${items.length} ${items.length===1?'solução':'soluções'}`;
    get('#catalog-empty').hidden=items.length>0;get('#catalog-grid').innerHTML=items.map(card).join('');
  }
  function renderDetail(id){
    const item=window.CATALOG.find(x=>x.id===id);if(!item){navigate('catalog',{replace:true});return;}
    activeSolution=item.id;previousFocus=document.activeElement;
    const labels=item.needs.map(id=>window.NEEDS.find(n=>n.id===id)?.label).filter(Boolean);
  ...
  function renderProcess(){get('#process-grid').innerHTML=window.PROCESS.map((step,i)=>`<article class="process-step"><span class="step-number">${String(i+1).padStart(2,'0')} <i></i></span><div><h2>${esc(step[0])}</h2><p>${esc(step[1])}</p></div></article>`).join('');}
